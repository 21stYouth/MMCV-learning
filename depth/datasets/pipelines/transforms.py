...
class depthmix(object):
    ...
    def __call__(self, results):
        ...
        depth1 = results["depth_gt"]

        img2 = results["img2"]

        depth2 = results["depth_gt2"]
        ...
        plt.subplot(2, 2, 3)
        plt.title("img1 after depthmix")
        plt.imshow(img1_ans) # 显示图片
        plt.subplot(2, 2, 4)
        plt.title("img2 after depthmix")
        plt.imshow(img2_ans) # 显示图片
        plt.show()

        results["img"] = img1_ans
        results["depth_gt"] = depth1_ans
...
class cutmix(object):
    ...
    def __call__(self, results):
        ...
        img1_ans = np.array(img1)
        img2_ans = np.array(img1)
        img3_ans = np.array(img1)
        depth1_ans = np.array(depth1)
        depth2_ans = np.array(depth1)
        depth3_ans = np.array(depth1)
        ...
        if depth1.shape == depth2.shape:
            ...
            for i in range(depth1.shape[0]):
                for j in range(depth1.shape[1]):
                    if i <= 0.5 * depth1.shape[0] and j <= 0.5 * depth1.shape[1]:
                        img1_ans[i][j] = 0
                        depth1_ans[i][j] = 50
            ...
            for i in range(depth1.shape[0]):
                for j in range(depth1.shape[1]):
                    if i <= 0.5 * depth1.shape[0] and j <= 0.5 * depth1.shape[1]:
                        img2_ans[i][j] = img2[i][j]
                        depth2_ans[i][j] = depth2[i][j]

            for i in range(depth1.shape[0]):
                for j in range(depth1.shape[1]):
                    img3_ans[i][j] = (img1[i][j] + img2[i][j]) / 2
                    depth3_ans[i][j] = (depth1[i][j] + depth2[i][j]) / 2
...
class cutout(object):
    ...
    def __call__(self, results):
        ...
        # have 0.1 probability to do the cutout
        # and 0.9 prob to not do
        if r != 0:
            return results
        ...
        h = results['depth_ori_shape'][0]
        w = results['depth_ori_shape'][1]
        mask = np.ones((h, w), np.uint8)

        self.length = min(h, w) // 4
        ...
        # plt.subplot(2, 2, 3)
        # plt.title("img_cutout")
        # plt.imshow(img)  # 显示图片
        # plt.subplot(2, 2, 4)
        # plt.title("depth_gt_cutout")
        # plt.imshow(depth_gt)  # 显示gt
        # plt.show()
...
class mixup(object):
    ...
    def __call__(self, results):
        ...
        img_ans = np.array(img1)
        depth_ans = np.array(depth1)
        ...
        if img1.shape == img2.shape:
            ...
            for i in range(img1.shape[0]):
                for j in range(img1.shape[1]):
                    img_ans[i][j] = img1[i][j] * lam + img2[i][j] * (1-lam)
                    depth_ans[i][j] = depth1[i][j] * lam + depth2[i][j] * (1-lam)
        ...
        # plt.subplot(2, 2, 3)
        # plt.title("mixup")
        # plt.imshow(img_ans) # 显示图片
        # plt.subplot(2, 2, 4)
        # plt.title("mixup_gt")
        # plt.imshow(depth_ans)  # 显示图片
        # plt.show()

        results["img"] = img_ans
        results["depth_gt"] = depth_ans
